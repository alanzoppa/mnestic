...
- Name: calendar.spec.ts >> Calendar Day Page >> should display notes section
- Location: e2e/calendar.spec.ts:82:7
...
Locator: locator('section').filter({ has: locator('h2:has-text(\'Notes\')') }).first().locator('h2:has-text(\'Notes\')')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('section').filter({ has: locator('h2:has-text(\'Notes\')') }).first().locator('h2:has-text(\'Notes\')')
