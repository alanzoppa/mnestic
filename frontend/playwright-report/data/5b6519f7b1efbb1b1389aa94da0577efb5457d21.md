...
- Name: calendar.spec.ts >> Calendar Day Page >> should display date title
- Location: e2e/calendar.spec.ts:62:7
...
Locator: getByRole('heading').filter({ hasText: /March|Friday/ }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading').filter({ hasText: /March|Friday/ }).first()
