...
- Name: calendar.spec.ts >> Calendar Day Page >> should display events section
- Location: e2e/calendar.spec.ts:74:7
...
Locator: locator('section').filter({ has: locator('h2:has-text(\'Events\')') }).first().locator('h2:has-text(\'Events\')')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('section').filter({ has: locator('h2:has-text(\'Events\')') }).first().locator('h2:has-text(\'Events\')')
